...
const bronzeMedalArt = loadImage('./assets/bronze-medal.png');
const silverMedalArt = loadImage('./assets/silver-medal.png');
const goldMedalArt = loadImage('./assets/gold-medal.png');
...
socket.on('init', ({ id, players, medal }) => {
  ...
  // Handle new medal gen
  socket.on('new-medal', newMedal => {
    item = new Collectible(newMedal);
  });
  ...
  // create current medal when logging in
  currPlayers = players.map(val => new Player(val)).concat(mainPlayer);
  item = new Collectible(medal);
...
const draw = () => {
  ...
  context.fillText('Medal Race', canvasCalcs.canvasWidth / 2, 32.5);
  ...
  // Draw current medal
  item.draw(context, { bronzeMedalArt, silverMedalArt, goldMedalArt });

  // Remove destroyed medal
  if (item.destroyed) {
    socket.emit('destroy-item', { playerId: item.destroyed, medalValue: item.value, medalId: item.id });
  }
